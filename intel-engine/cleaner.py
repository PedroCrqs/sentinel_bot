...
def _purge_opportunities_file(filepath: str):
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            lines = f.readlines()
    except FileNotFoundError:
        print(f"[PURGE] {filepath} não encontrado, pulando.")
        return
    ...
    print(f"[PURGE] {filepath}: {removed} removidas, {len(kept)} mantidas.")


def purge_opportunities():
    _purge_opportunities_file(OPPORTUNITIES_FILE)
    _purge_opportunities_file(SELF_OPPORTUNITIES_FILE)
