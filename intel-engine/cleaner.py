...
def _clean_and_dedup_opportunities_file(filepath: Path, state_key: str, label: str):
    ...
    _write_jsonl(filepath, kept)

    kept_ids = [o.get("id") for o in kept if o.get("id")]
    sync_dispatch_state(kept_ids, state_key)
...
def clean_and_dedup_opportunities():
    _clean_and_dedup_opportunities_file(OPPORTUNITIES_FILE, "sent", "opportunities.jsonl")


def clean_and_dedup_self_opportunities():
    _clean_and_dedup_opportunities_file(
        SELF_OPPORTUNITIES_FILE, "sent_self", "self_opportunities.jsonl"
    )
